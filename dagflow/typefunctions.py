...
def check_nonzero_inputs(node: NodeT) -> None:
    """Checking if the node has inputs"""
    if len(node.inputs) == 0:
        raise TypeFunctionError(f"Cannot use '{node.name}' with zero inputs!")


def eval_output_dtype(node: NodeT, outputkey: Union[int, str] = "result") -> None:
...
def copy_input_dtype_to_output(
    node: NodeT,
    inputkey: Union[int, str] = 0,
    outputkey: Union[int, str] = "result",
) -> None:
...
def combine_inputs_shape_to_output(
    node: NodeT, outputkey: Union[int, str] = "result"
) -> None:
